...
export default function Festival() {
    const apiKey = process.env.NEXT_PUBLIC_PUBLIC_API;
    ...
    return (
        <div className="w-full h-full p-5 flex flex-col items-center gap-3">
            ...
            <div className="w-9/10 p-5 grid 2xl:grid-cols-4 lg:grid-cols-3 gap-6">
                {
                    card && !loading ? card.map((item, i) =>
                        <Link key={i} href={`/festival/contents?item=${encodeURIComponent(JSON.stringify(item))}`} >
